...
namespace Lattice.Editor.Views
{
    ...
    public class BaseGraphView : GraphView, IDisposable
    {
        ...
        public virtual void LoadGraph(BaseGraph graph)
        {
            ...
            if (malformedDetails != null)
            {
                malformedTextBox = AssetDatabase
                                   .LoadAssetAtPath<VisualTreeAsset>(
                                       "Packages/com.pon.lattice/Editor/UI/InvalidGraph.uxml").CloneTree();
            ...
            ConnectorListener = new BaseEdgeConnectorListener(this);
        ...
        {
            foreach (var serializedEdge in Graph.NonDanglingEdges())
            {
                ...
                Port inputPort = inputNodeView?.GetPort(serializedEdge.toPortIdentifier);
                Port outputPort = outputNodeView?.GetPort(serializedEdge.fromPortIdentifier);
        ...
        /// <summary>Add the New Group entry to the SharedContext menu</summary>
        ...
        /// <summary>Add the 'New Sticky Note' entry to the context menu.</summary>
        protected virtual void BuildStickyNoteContextualMenu(ContextualMenuPopulateEvent evt, int menuPosition)
        {
        ...
        /// <summary>Add the Select Asset entry to the SharedContext menu</summary>
        ...
        public EdgeView CreateNewEdge(PortView inputPortView, PortView outputPortView, bool autoDisconnectInputs = true)
        {
            ...
            var serializableEdge = SerializableEdge.CreateNewEdge(Graph, inputPort, outputPort);

            var edgeView = new EdgeView();

            edgeView.userData = serializableEdge;
        ...
        public bool Connect(EdgeView e, bool autoDisconnectInputs = true)
        {
            ...
            var inputPort = ((BaseNodeView)inputPortView.node).NodeTarget.GetPort(inputPortView.Identifier);
            var outputPort = ((BaseNodeView)outputPortView.node).NodeTarget.GetPort(outputPortView.Identifier);
